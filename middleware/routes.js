...
//		    		});
        ...
        socket.on('sendMsgToRoom',function(data_server){
	    	/*
	    		calling saveMsgs to save messages into DB.
	    	*/
	    	helper.insertMsg(data_server,connection,function(result){
	    		/*
	    			Chechking users is offline or not
	    		*/
                users.forEach(function(element, index, array){
                    io.to(element.socketId).emit('getRoomMsg',result.room_id);
                });
	    	});	    	  	    
	    });
	    /*
	...
	*/
    ...
    app.post('/get_room_msgs', function(req, res){
		/*
	    	Calling 'getRoomMsgs' to get messages
	    */
		helper.getRoomMsgs(req.body,connection,function(result){
			res.write(JSON.stringify(result));
			res.end();
		});		
	});
    /*
		post to handle get_chats_rooms request
	*/
	app.post('/get_chats_rooms', function(req, res){
		/*
	    	Calling 'getChatRooms' to get user chat list
	    */
		helper.getChatRooms(req.body.uid, connection,function(dbRooms){
	       res.write(JSON.stringify(dbRooms));
	       res.end();	
		});	
	});
